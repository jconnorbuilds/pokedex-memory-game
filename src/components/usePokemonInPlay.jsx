...
export default function usePokemonInPlay(allPokemonInGeneration, needsStarters, level) {
  ...
  const getNextPokemonInPlay = null;

  useEffect(() => {
    const LEVELS = { easy: 4, medium: 8, hard: 12 };

    if (!allPokemonInGeneration || !needsNewPkmn) return;
    console.log(...allPokemonInGeneration.slice(0, 5));
    ...
    const selectedPokemon = needsStarters ? [...allPokemonInGeneration.slice(0, 3)] : [];
